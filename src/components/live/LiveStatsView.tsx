...
export function LiveStatsView() {
  ...
  const isLoading =
    sessionQuery.isLoading || driversQuery.isLoading || statsQuery.isLoading;
  const hasError =
    sessionQuery.isError || driversQuery.isError || statsQuery.isError;
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-white/60">
              Detailed performance metrics including fastest laps, top speeds,
              pit stop strategies, and sector analysis.
            </p>
          </div>
          <StatusPill
            loading={isLoading}
        ...
        {statsQuery.isError && (
          <div className="mt-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4">
            ...
            <p className="mt-1 text-xs text-amber-200/80">
              The OpenF1 API is currently rate-limiting requests. Data will
              retry automatically in 30 seconds. Try refreshing the page in a
              moment or visit the Archive page for historical data.
            </p>
          </div>
        )}
      </header>
